#!/usr/bin/env python3
"""
GeckosHUB - User Version (Simplified)
...
from PySide6.QtWidgets import (QApplication, QMainWindow, QTabWidget, QVBoxLayout, 
                               ...
                               QHBoxLayout, QMessageBox, QFrame, QCheckBox, QListWidget,
...
class UpdateChecker(QThread):
    ...
    def __init__(self):
        super().__init__()
        # ÄNDERE DIESE URL zu deinem GitHub Repository
        self.github_url = "https://raw.githubusercontent.com/DEIN_USERNAME/DEIN_REPO/main"
...
class LicenseKeyDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.license_input.setPlaceholderText(f"{self.tab_name.upper()}-XXXXXXXX-XXXXXXXX")
    ...
    def validate_license(self):
        ...
        if license_key.startswith(f"{tab_prefix}-") or license_key.startswith("GECKO-"):
            ...
            # Für Demo-Zwecke akzeptieren wir Keys mit korrektem Format
...
class ProgramTab(QWidget):
    ...
    def load_programs(self):
        ...
        if not programs:
            # Fallback: Zeige Info dass keine Programme konfiguriert sind
    ...
    def launch_program(self, item=None):
        ...
        if program is None:
            return  # Info-Item oder kein Programm
...
class GeckosHUB(QMainWindow):
    ...
    def load_tabs_config(self):
        ...
        try:
            if os.path.exists("tabs_config.json"):
                ...
                # Fallback zu Standard-Tabs
                self.tabs_config = {
                    "tabs": [
                        {
                            "name": "Example Tab",
                            "icon": "🎮",
                            "description": "No configuration found. Please check if tabs_config.json exists.",
    ...
    def setup_tabs(self):
        ...
        if not tabs:
            # Fallback Tab wenn keine Konfiguration vorhanden
